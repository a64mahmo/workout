...
async def get_current_user(user_id: str, db: AsyncSession):
...
async def get_fitbit_auth_url(user_id: str):
    state = str(uuid.uuid4())
    url = fitbit_service.get_auth_url(state)
    return {"url": url}

@router.post("/callback")
async def fitbit_callback(data: FitbitCallback, user_id: str, db: AsyncSession = Depends(get_db)):
...
async def disconnect_fitbit(user_id: str, db: AsyncSession = Depends(get_db)):
    user = await get_current_user(user_id, db)
...
@router.get("/status")
async def fitbit_status(user_id: str, db: AsyncSession = Depends(get_db)):
    user = await get_current_user(user_id, db)
